...
To select the lucky winner once the contest ended, we needed a method that was fair and transparent. Here’s how we did it: We wrote a simple script to assign each entrant a number. We then flipped a coin ten times, each time recording the result as either a 1 (heads) or 0 (tails). The result was a 10 digit binary representation, which when converted to a decimal number gave us a completely random result between 1 and 1,024.
