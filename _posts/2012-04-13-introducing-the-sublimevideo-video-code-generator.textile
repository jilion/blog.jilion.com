...
Ever since we first launched the SublimeVideo Beta, the main cause of integration issues encountered by our users has been the problems they run into when creating their own &lt;video&gt; elements.

We designed the Video Code Generator to streamline this process, and are pleased to announce that it is *available now to all SublimeVideo users* directly from their 'My Sites' page.
...
A common example of a &lt;video&gt; element being configured incorrectly would be when its width or height values don’t retain the source’s aspect ratio, resulting in black bars (letterboxes/pillarboxes) around the video.

The Video Code Generator detects the original size and aspect ratio of the source file(s) and automatically provides the user with correctly proportioned width and height values. It also fixes other common configuration issues including incorrect ordering of the &lt;source&gt; elements and ensuring a &lt;source&gt;’s type attribute is set to the right value.
...
<img src="http://blog.jilion.com.s3.amazonaws.com/uploads/2012/04/sublimevideo-seal-of-approval.png" alt="SublimeVideo - Seal Of Approval" style="float:left;margin:5px 20px 0 0" /> In addition to helping users resolve these issues, the Video Code Generator also helps them set up SublimeVideo-specific features such as Lightboxes, HD Switching, Embeds and video metadata to optimize their videos for SublimeVideo’s Universal, Real-Time Statistics platform. 
